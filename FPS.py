...
def net_boost_ultra():
    ...
    wr('/proc/sys/net/core/rmem_max', 2147483647)
    wr('/proc/sys/net/core/wmem_max', 2147483647)
    wr('/proc/sys/net/core/rmem_default', 134217728)
    wr('/proc/sys/net/core/wmem_default', 134217728)
    wr('/proc/sys/net/core/netdev_max_backlog', 8388608)
    wr('/proc/sys/net/core/somaxconn', 65535)
    wr('/proc/sys/net/ipv4/tcp_rmem', '4096 87380 2147483647')
    wr('/proc/sys/net/ipv4/tcp_wmem', '4096 65536 2147483647')
    ...
    wr('/proc/sys/net/ipv4/tcp_keepalive_time', 15)
    wr('/proc/sys/net/ipv4/tcp_keepalive_probes', 5)
    wr('/proc/sys/net/ipv4/tcp_keepalive_intvl', 5)
    ...
    run('ip link set mtu 1500 dev rmnet_data0 || true')
    run('ip link set mtu 1500 dev wlan0 || true')
    run('ip link set mtu 1500 dev eth0 || true')
...
def init_clean():
    ...
    run('logcat -c')
    run('dmesg -c || true')
    run('rm -rf /cache/* /mnt/cache/* /data/cache/* /data/local/tmp/* /data/tmp/*')
    run('rm -rf /data/system/dropbox/* /data/anr/* /data/tombstones/*')
    run('rm -rf /data/system/usagestats/* /data/system/sync/*')
    run('rm -rf /data/misc/logd/* /data/misc/profiles/* /data/misc/textclassifier/*')
    run('rm -rf /data/system/notification_log.xml')
    run('rm -rf /data/dalvik-cache/* /cache/dalvik-cache/* /mnt/dalvik-cache/*')
